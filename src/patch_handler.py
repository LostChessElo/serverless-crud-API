...
def patch_handler(event: APIGatewayProxyEventV1) -> dict[str, Any]: 
    try:
        ...
        name: Any = body.get("name") if body.get("name") is not None else old_note["name"]
        description: Any = body.get("description") if body.get("description") is not None else old_note["description"]
