...
def test_vaid_rq_body_returns_200(mock_table):
    context = Context()
    event = {"body": '{"name": "test note", "description": "Note to test"}'}
...
def test_note_already_exists_returns_200(mock_table):
    mock_table.put_item(
        Item={"id": "123ABC", "name": "test note", "description": "Note to test"}
    )
    context = Context()
    event = {"body": '{"name": "test note", "description": "Note to test"}'}
    response = post_handler(event, context)
...
def test_empty_name_returns_400(mock_table):
    context = Context()
    event = {"body": '{"name": "", "description": "Note to test"}'}
    response = post_handler(event, context)
    body = json.loads(response.get("body"))
...
def test_no_name_returns_400(mock_table):
    context = Context()
    event = {"body": '{ "description": "Note to test"}'}
    response = post_handler(event, context)
    body = json.loads(response.get("body"))
    assert response.get("statusCode") == 400
    assert body.get("message") is not None
